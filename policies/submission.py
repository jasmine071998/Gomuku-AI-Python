...
class AdvancedGomokuAI:
        ...
        for dr, dc in directions:
            ...
            for i in range(1, 5):
                ...
                if not self.is_valid_pos(r, c):
                    break
                if state.board[0,r, c] == player:  # Ensure r and c are single integers
            ...
            for i in range(1, 5):
                ...
                if len(pattern_stones) >= 3:  # Consider a pattern if it includes at least 3 stones
    ...
    def immediate_threats(self, state):
        for player in [1, 2]:  # Assuming 1 and 2 are player identifiers
            for r in range(self.board_size):
                for c in range(self.board_size):
        ...
        for dr, dc in directions:
            ...
            for i in range(1, self.win_size):
                ...
                if 0 <= nr < self.board_size and 0 <= nc < self.board_size and state.board[player, nr, nc] == 1:
                    ...
                    break
            if count >= self.win_size:  # Win condition met
                return True
        return False
